...
fn encrypt_decrypt_round_trip() {
    let bytes = Message::encrypt(&KEY, "user", "secret").unwrap();
...
fn empty_plaintext_round_trip() {
    ...
    let bytes = Message::encrypt(&KEY, "user", "").unwrap();
    let msg = Message::decrypt(&KEY, &bytes).unwrap();
...
fn tampered_ciphertext_fails() {
    ...
    let mut bytes = Message::encrypt(&KEY, "user", "secret").unwrap();
    let mid = bytes.len() / 2;
    bytes[mid] ^= 0xFF;
...
fn nonce_is_randomized_per_message() {
    ...
    let a = Message::encrypt(&KEY, "user", "secret").unwrap();
    let b = Message::encrypt(&KEY, "user", "secret").unwrap();
